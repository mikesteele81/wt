  ...
  where
    ...
    dispatch "GET" ["forward"] = do
        ...
        eres <- lift $ try $ OpenId.getForwardUrl googleIdent complete' Nothing
            ...
            , ("openid.ns.ax.required", "email,firstname,lastname")
...
completeHelper gets' = do
        ...
        let onSuccess oir = do
                ...
                mfirst <- lookupGetParam "openid.ext1.value.firstname"
                mlast  <- lookupGetParam "openid.ext1.value.lastname"
                case (memail, "https://www.google.com/accounts/o8/id" `T.isPrefixOf` ident) of
                    (Just email, True) -> setCreds True $ Creds "googleemail" email
                        ( maybe id ((:) . ("firstname",)) mfirst
                        $ maybe id ((:) . ("lastname",)) mlast
                        $ [])
                    (_, False) -> do
